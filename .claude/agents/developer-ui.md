...
- Módulos por-proyecto son rutas propias bajo `src/app/proyectos/[id]/<modulo>/page.tsx`, no `Tabs` dentro de una mega-página. El layout compartido (`src/app/proyectos/[id]/layout.tsx`) solo pone el header del proyecto — **no** hay una nav secundaria/tabs ahí (se quitó el 2026-06-26 por ser redundante con el sidebar, que ya escala mejor a ~20 módulos). La navegación entre módulos del proyecto es únicamente el sidebar; sumá también una card de acceso en `src/app/proyectos/[id]/page.tsx` (Resumen), siguiendo el patrón de la constante `MODULES` ahí.
